...
| 43 | Sweep visualization in UI | Add a Sweep Results tab to the H2H page. Visualize: (1) convergence plot (best WR over trials), (2) parameter importance (correlation of each param with WR), (3) parallel coordinates plot of top-10 vectors, (4) param distribution heatmaps for good vs bad trials (For this i want to see the min and max of every param that was defined (move those definitions to a json from the SweepMain.java for better usability?) and then for the top x runs points on each of those rows (connected to each other to form a line per param-try) or use a better form of visualisation). Data source: `data/sweep-results.json`. |
