...
- `rankPurchasableProjects` — sorted list of all affordable cards by ROI; candidate list combines `unbuilt_projects` (regular establishments) and any unowned Großprojekte from `ProjectLoader.getAllProjects()`; computes MC baseline once and reuses it across all candidates.
