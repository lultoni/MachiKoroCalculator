...
- `probability.TurnRecord` — immutable record of one turn (playerIndex, roll, bought project or null, isDoubles boolean, coinDeltas int[] or null, swappedAway/swappedIn Project or null). The 3-arg, 4-arg, and 5-arg constructors default swap fields to null for backward compatibility. `GameSession.applyTurn` always stores an augmented copy with `coinDeltas` filled from `computeAllDeltasForRoll`. `GameSession.applyBürohausSwap` patches the last TurnRecord with swap fields.
- `probability.GameSession` — wraps a mutable `GameState` with a full `ArrayList<TurnRecord>` history. Fields: `bonusTurnPending` (runtime flag), `effectiveTurnCount` (increments only on non-bonus turns). Methods: `applyTurn`, `applyBürohausSwap` (executes bürohaus swap + patches last TurnRecord), `undoLastTurn`, `toSnapshot` (→ builder), `fromSnapshot` (builder → new session), `nextPlayerIndex` (uses `effectiveTurnCount % n`; returns same player when `bonusTurnPending`), `isBonusTurnPending`. Bidirectional turn-by-turn ↔ snapshot conversion. `save`/`load` are thin wrappers that delegate to `GameSessionPersistence`.
...
- `probability.RolloutTree` — Stufe-1 Expectimax rollout tree. `evaluate(gs, pi, depth, topK)` returns `RolloutResult(bestAction, expectedWinProb, allValues)`. Candidate pruning via `portfolioDeltaEV`, leaf evaluation via `computeBaselineWinProb`, opponent simulation via `GameSimulator.boltzmannBuy(T=0.7)`. Handles Bahnhof/Freizeitpark/Funkturm special cases. Constants: `EPSILON_CONVERGE=0.01`, `ENDGAME_COINS_THRESHOLD=8`.
...
- `gui.newui.Strings` — central i18n string registry. `Locale` enum (`DE`/`EN`); `setLocale()`; `isDE()`. All user-visible strings accessed through static methods here. Card names/descriptions in `Project.getLocalizedName()` / `getLocalizedDescription()` delegate to `Strings.isDE()`.
- `gui.newui.SetupWindow` — game setup screen (DE/EN radio buttons, player count, names, Start button). Switching locale calls `Strings.setLocale()` then `rebuildUI()` in-place. Builds the initial `GameSession` from a `GameStateBuilder` and opens `MainWindow`.
- `gui.newui.MainWindow` — three-column window: left = "Current Turn Tracker" (BorderLayout: controls in NORTH, history JScrollPane in CENTER that fills remaining space; roll spinner with dynamic 1–6/1–12 range, "Doubles?" checkbox shown when active player owns Bahnhof+Freizeitpark, buy dropdown, Confirm Turn, Undo, Snapshot button, colored history log with per-player coin-delta lines and doubles badge); center = "Card Details" (card name, EV/round, ROI, risk, optional win-prob delta, roll-outcome preview per player); right = full ranking table with 5 tabs: Affordable, Not Affordable, All, Assistant, Rollout. Rollout tab: depth spinner (1–3), top-K spinner (2–8), Run button (SwingWorker), result table sorted by win-prob with color coding. Win-probability delta is on-demand (toggle button, not shown by default); Deep Analysis toggle enables MC backend only — win-prob column visibility is independent. **The buy dropdown and ranking always use post-roll coins** (`postRollState()` applies `computeAllDeltasForRoll` to a state copy before ranking). The coin label shows "N → M (after roll)" when the roll changes the active player's coins. Changing the roll spinner immediately re-evaluates the buy list and ranking. Has a `JMenuBar` with a "Language/Sprache" menu containing DE/EN `JRadioButtonMenuItem`s; switching calls `Strings.setLocale()` then `buildUI()` + `refreshAll()` in-place. Card lookup from combo labels uses `projectFromLabel` (reverse lookup by `getLocalizedName()`) to handle both locales correctly.
- `gui.newui.SnapshotDialog` — modal dialog for editing the full game state mid-session (tabs per player with coin spinner, JSpinner(0–7) for weizenfeld/bäckerei, JSpinner(0–6) for other blau/grün/rot multi-copy cards, JCheckBox for lila/gelb unique cards). On apply, calls `MainWindow.replaceSession()` to re-root the session at the snapshot.
- `gui.newui.BoundedSpinner` — extends `JSpinner`; disables the increment button when the model is at its maximum and the decrement button when at its minimum. Used for all spinners in MainWindow and SnapshotDialog.
...
- `rankPurchasableProjects` — sorted list of all affordable cards by ROI; candidate list combines `unbuilt_projects` (regular establishments) and any unowned Großprojekte from `ProjectLoader.getAllProjects()`; computes MC baseline once and reuses it across all candidates. After ROI sort, calls `adaptiveMCRefinement` (private) for Top-k adaptive MC budget when `mcSimulations > 0`.
- `adaptiveMCRefinement` (private) — Stufe-3 adaptive MC budget: Top-k=`MC_TOP_K(5)` candidates; equal-split sims when spread ≤ `MC_EQUAL_BUDGET_EPSILON(0.02)` or no dominant leader; chasers-only when leader advantage > `MC_DOMINANT_LEAD_THRESHOLD(0.05)`; `MC_SIMS_PER_CANDIDATE_EQUAL=2500` sims per validated candidate. Overwrites `entry.winProbDelta` for validated candidates.
