...
- `probability.TurnRecord` — immutable record of one turn (playerIndex, roll, bought project or null, isDoubles boolean). The 3-arg constructor defaults `isDoubles=false` for backward compatibility.
...
- `gui.newui.MainWindow` — three-column window: left = "Current Turn Tracker" (roll spinner with dynamic 1–6/1–12 range, "Doubles?" checkbox shown when active player owns Bahnhof+Freizeitpark, buy dropdown, Confirm Turn, Undo, Snapshot button, colored history log with doubles badge); center = "Card Details" (card name, EV/round, ROI, risk, optional win-prob delta, roll-outcome preview per player); right = full ranking table (sortable columns, color-coded values via `NumericCellRenderer`, GPs included). Win-probability delta is on-demand (toggle button, not shown by default). **The buy dropdown and ranking always use post-roll coins** (`postRollState()` applies `computeAllDeltasForRoll` to a state copy before ranking). The coin label shows "N → M (after roll)" when the roll changes the active player's coins. Changing the roll spinner immediately re-evaluates the buy list and ranking.
