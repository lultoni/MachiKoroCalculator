...
- `gui.newui.MainWindow` — three-column window: left = turn input (roll spinner with dynamic 1–6/1–12 range, buy dropdown, Confirm Turn, Undo, Snapshot button, history log); center = top recommendation (card name, EV/round, ROI, risk, optional win-prob delta, roll-outcome preview); right = full ranking table (sorted by ROI, color-coded by card type). Win-probability delta is on-demand (toggle button, not shown by default). **The buy dropdown and ranking always use post-roll coins** (`postRollState()` applies `computeAllDeltasForRoll` to a state copy before ranking): Machi Koro turn order is roll → collect → buy, so affordability is assessed after income is received. The coin label shows "N → M (after roll)" when the roll changes the active player's coins. Changing the roll spinner immediately re-evaluates the buy list and ranking.
- `gui.newui.SnapshotDialog` — modal dialog for editing the full game state mid-session (tabs per player with coin spinner, JSpinner(0–6) for blau/grün/rot multi-copy cards, JCheckBox for lila/gelb unique cards). On apply, calls `MainWindow.replaceSession()` to re-root the session at the snapshot.
