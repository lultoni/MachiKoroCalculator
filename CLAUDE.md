...
- `probability.TurnRecord` — immutable record of one turn (playerIndex, roll, bought project or null, isDoubles boolean, coinDeltas int[] or null). The 3-arg and 4-arg constructors default `coinDeltas=null` and `isDoubles=false` for backward compatibility. `GameSession.applyTurn` always stores an augmented copy with `coinDeltas` filled from `computeAllDeltasForRoll`.
- `probability.GameSession` — wraps a mutable `GameState` with a full `ArrayList<TurnRecord>` history. Fields: `bonusTurnPending` (runtime flag), `effectiveTurnCount` (increments only on non-bonus turns). Methods: `applyTurn`, `undoLastTurn`, `toSnapshot` (→ builder), `fromSnapshot` (builder → new session), `nextPlayerIndex` (uses `effectiveTurnCount % n`; returns same player when `bonusTurnPending`), `isBonusTurnPending`. Bidirectional turn-by-turn ↔ snapshot conversion. `save`/`load` are thin wrappers that delegate to `GameSessionPersistence`.
