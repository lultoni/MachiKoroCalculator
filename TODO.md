...
| # | Task | Type | Notes |
|---|------|------|-------|
| ~~**17c**~~ | ~~**Continuous thinking engine infrastructure**~~ | ~~Dev (critical)~~ | ~~**DONE.**~~ ContinuousWorker interface + 5 engine implementations (MCTS tree persistence + TreeNavigator, FlatMC/Creator candidate accumulators, Expectimax iterative deepening, HeuristicEv instant). ContinuousEvaluator wrapper with flag-based "letter on desk" control. Timekeeper for minThinkTimeMs. Server-side PlayerVsAiController + 3 API endpoints. Frontend usePlayerVsAi hook + PlayerVsAiPanel. 16 tests. |
